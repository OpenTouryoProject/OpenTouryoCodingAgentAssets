...
opentouryo-app-design             実効65L tok~2697  完了（**アプリ設計のポイント＝設計フェーズの地図/チェックリスト**。spec/plan の設計段で決める事項〔レイヤ/例外/Tx/データアクセス/共有情報/画面/セッション・セキュリティ/認証/ログ/国際化/呼出・非同期/コメント規約〕を各実装スキルへ割り付け。未整備領域〔帳票/印刷/メール・バッチ処理方式・WebAPI・性能/高信頼性・組込/ワークフロー〕は配下の子スキル/references で順次整備。出典＝公式「アプリケーション設計のポイント」目次。**親スキル**〔子は個別課題ごとに追加〕）
                                  ...
                                  └ references/internationalization.md 実効55L tok~1356  国際化 i18n／地域化（対応項目＝文言/書式/カレンダー/タイムゾーン/和暦・元号/双方向 RTL/文字コード。.NET＝`CultureInfo`〔`CurrentCulture`=書式・`CurrentUICulture`=UI 表示を分ける〕/`.resx`/`JapaneseCalendar`/UTC 保持。**★ 推奨 C/S 設計**：UI 表示＝クライアント可変／ログ＝サーバ固定／書式＝クライアント or 統一。**OpenTouryo 対応部分**：メッセージ カルチャ〔`FxExceptionMessageCulture`/`MSGDefinition_<culture>.xml`=message〕・**`GMTMaster`**〔public・ローカル⇔UTC〕・**`FormatConverter`**〔public・丸め/桁区切り〕・`StringChecker`/`JIS2k4Checker`〔character-encoding〕。※`MyTimeZone` は internal。和暦は net48 `EnforceJapaneseEraYearRanges`/Core は別。実ソース＋MS Learn で裏取り）
