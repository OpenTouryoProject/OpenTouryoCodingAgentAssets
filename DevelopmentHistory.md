...
opentouryo-comment-convention     実効 60L tok~1284  完了（**コメント／ファイルヘッダ規則**。新規 `.cs`/`.vb` の先頭に所属機能名バナー＋クラス情報ブロック〔クラス名/日本語名/作成日時/作成者/更新履歴表〕を言語コメントで〔C#=`//*`・VB=`'*`・バナー=接頭辞＋`*`×82・UTF-8 BOM〕。クラス/メソッドは `/// <summary>` XML doc、インラインは言語仕様。`.aspx`/`.master` マークアップは対象外・自動生成物はツール任せ。実サンプル `LayerB.cs`/`LayerB.vb` で裏取り）
