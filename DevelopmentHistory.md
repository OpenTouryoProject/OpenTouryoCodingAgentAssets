...
**実装と食い違う箇所を発見した。ソースを正とすること。**

| 項目 | 2016年ドキュメント | 実装（`BaseLogic.cs`） |
| --- | --- | --- |
| フレームワーク例外・一般例外のB層での扱い | 「例外をリスローする」 | **リスローしない**（`UOC_ABEND` で必要に応じてリスロー） |

また `FrameworkException` は `BaseLogic` で個別に `catch` されず、`catch (Exception)` に落ちる。
