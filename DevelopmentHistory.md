...
opentouryo-project-setup          実効250L tok~5100  完了（立ち上げ入口。①〜⑦。③は build へ委譲。SKILL-FEEDBACK 反映。★目安上限付近＝WS 詳細は on-demand ファイルへ退避）
                                  ├ samples/webservices.md 56L tok~1200  WS/3層の共通機構（on-demand・サンプル横断で共有）：(A)取り出し/参照/Build\配置・(B)2層化・core 実用不可・MAX_PATH
                                  └ samples/webforms.md    28L tok~550   Web Forms 固有のみ（on-demand。cc 画面の CS0246・(B)画面差し替え・config二段）。共通は samples/webservices.md 参照
