...
opentouryo-app-design             実効64L tok~2374  完了（**アプリ設計のポイント＝設計フェーズの地図/チェックリスト**。spec/plan の設計段で決める事項〔レイヤ/例外/Tx/データアクセス/共有情報/画面/セッション・セキュリティ/認証/ログ/国際化/呼出・非同期/コメント規約〕を各実装スキルへ割り付け。未整備領域〔帳票/印刷/メール・バッチ処理方式・WebAPI・性能/高信頼性・組込/ワークフロー〕は配下の子スキル/references で順次整備。出典＝公式「アプリケーション設計のポイント」目次。**親スキル**〔子は個別課題ごとに追加〕）
