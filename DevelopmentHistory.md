...
### 2.7 ツールのスキル化：基準は「エージェントが直接できるか」（2026-07・作者判断）

**判断基準：エージェントがそのツールの機能を直接できるか。**
GUI ツールはそのままではスキル化が困難。ただし**周辺ライブラリを活用してエージェントがそのツールの
機能を生成・再現できるならスキル化が可能**（例：動的 SQL のテスト実行）。
一方、**エージェントが直接し難い複雑な処理を含むなら、CLI 化することでスキル化が可能になる**
（例：D層 Dao 自動生成）。

| ツール | スキル化の手段 | 理由 |
| --- | --- | --- |
| `DPQuery_Tool`（動的クエリ分析ツール） | **直接スキル化**（CLI 化不要） | エージェントは `.sql` / `.xml` を**直接書けて Dao 経由で実行・テストできる**＝周辺ライブラリで機能を再現できる。ツールの価値は「人が GUI で試験実行する」ことで、エージェントの作業フローとは前提が違う |
| **D層自動生成ツール（墨壺）** | **CLI 化してスキル化**（本体 Issue [#508](https://github.com/OpenTouryoProject/OpenTouryo/issues/508) 起票済み・2026-07-17） | **ランタイムから実行できない処理**（DB スキーマの読み取り → Dao / DTO / SQL のコード生成）を含む。エージェントが直接できないので、CLI 化すれば呼べる＝スキル化できる |
| データ・メンテナンス画面自動生成（ASP.NET） | **未着手・スコープ外**（将来 CLI 化候補） | doc0（Index）で存在を確認した既知の GUI ツール。墨壺同様「エージェントが直接できない生成処理」を含むので将来 CLI 化してスキル化する候補ではあるが、画面生成は現状スコープ外 |

**教訓：人間向け GUI ツールを機械的にエージェント向けへ移植しない。**
エージェントが**周辺ライブラリで元の成果物（コード）を直接扱える／機能を再現できるなら、そのままスキル化できる**（`DPQuery_Tool`）。
逆に**ツールが手作業では再現しにくい複雑な処理を持つなら、CLI 化してスキル化する**（自動生成ツール）。
...
### 2.8 横断的な設計トピックは `opentouryo-app-design/references/` に置く（単体スキル化しない）

**単一の実装スキル ホームを持たない横断的な設計トピックは、単体スキルに切り出さず
`opentouryo-app-design/references/*.md` に on-demand 参照として置く**（2026-07-27・作者方針）。

判断の背景（本セッションで確定）：

- `opentouryo-app-design` は**設計フェーズの地図（親スキル）**。§2.5 は「単体スキルへ分割する基準」だが、
  こちらは逆に**「分割せず app-design の配下に束ねる」基準**＝別軸。
- 対象は **config・複数の実装スキル・親クラス2 にまたがり、単一の実装スキル ホームが無いトピック**。
  例：`timeout-values.md`（HTTP/executionTimeout/DB/ロック/セッションを横断）、`file-upload-download.md`
  （P層＋config＋auth）、`cache-control.md`（`FxCacheControl`＋D層 SQL キャッシュ＋Core）、
  `illegal-operation-prevention.md`（config スイッチ＋screen-transition＋webforms-screen＋MyBaseController）。
- **`references/` は SKILL トークン予算に含まれない**（§トークン規律）。横断トピックの分量が増えても
  親 SKILL を圧迫せず、必要なときだけ読み込まれる。
- **実装の権威は既存スキル側に残す**。reference は「地図＋ポインタ」で、config キー等の一次情報は
  `opentouryo-config` 等を指す（defense-in-depth の重複は横断ルールに限る＝§4.6 ⑦）。

**単体スキルへ昇格する条件（今は該当せず）**：reference に**コピペ用スニペット**（例：不正操作防止の
MyBaseController UOC カスタマイズ＝ログ出力／セッション解放／`CanCheckIllegalOperation` の書き分け）が
必要になり、実装コードの分量が設計マップを超えたとき。その場合も**関連機能を一括**でスキル化する
（不正操作防止の例：二重送信防止・ボタン履歴・ブラウザ/ウィンドウ別 Session 領域を含む「Web セキュリティ実装」
として。単独の切り出しは非対称を生むので避ける）。

裏取り（本セッション）：「Web アプリの不正操作」4機能のうち**単体スキルは画面遷移制御
（`opentouryo-screen-transition`）だけ**。残り3つ（不正操作防止・二重送信防止・ボタン履歴/Session 領域）は
**`opentouryo-config` の P層セキュリティ スイッチ**として実装記述済み（`FxDoubleTransmissionCheck`＝
`onSubmit="return Fx_OnSubmit();"` の JS 仕込み〔`BaseController.cs` L672〕はクライアント側、
不正操作防止の Request Ticket 検出〔同 L693〕はサーバ側）。この非対称ゆえ不正操作防止だけの単体スキル化は見送り。
...
```
opentouryo-layer-p-mvc            実効287L tok~3959  完了（節「MVC に無い Web Forms 専用のP層機能」＝不正操作防止/二重送信/画面遷移/窓別Session は Web Forms 専用。キャッシュ制御のみ MVC も可）
opentouryo-layer-p-webforms-screen 実効181L tok~3256  完了（画面の新規作成）
opentouryo-layer-p-webforms-event  実効202L tok~4170  完了（イベント実装。GridView/ListView/Repeater を .aspx＋コードビハインドで系統別に統一・SKILL からリンク）
opentouryo-webforms-dialog         実効168L tok~2765  完了（子画面表示・ダイアログ。ブラウザ・ウィンドウ別／親画面別 Session 領域＝入れ子2層）
opentouryo-layer-p-winforms-screen 実効118L tok~1859  完了（画面の新規作成）
opentouryo-layer-p-winforms-event  実効139L tok~3045  完了（イベント実装）
opentouryo-p-call-business        実効200L tok~3510  完了（P層→B層呼出し・横断）
opentouryo-richclient-async       実効98L tok~2113  完了（リッチクライアントの非同期呼び出し）
opentouryo-common-parts           実効128L tok~2351  完了（用途→共通部品のインデックス）
opentouryo-app-design             実効64L tok~2922  完了（**アプリ設計のポイント＝設計フェーズの地図/チェックリスト**。spec/plan の設計段で決める事項〔レイヤ/例外/Tx/データアクセス/共有情報/画面/セッション・セキュリティ/認証/ログ/国際化/呼出・非同期/コメント規約〕を各実装スキルへ割り付け。未整備領域〔帳票/印刷/メール・バッチ処理方式・WebAPI・性能/高信頼性・組込/ワークフロー〕は配下の子スキル/references で順次整備。出典＝公式「アプリケーション設計のポイント」目次。**親スキル**〔子は個別課題ごとに追加〕）
                                  └ references/timeout-values.md   実効25L tok~883  タイムアウト値の設計（**呼び出し元ほど長く**。① HTTP〔100s〕>② `executionTimeout`〔110s〕>③ `ConnectionTimeout`〔15s〕/④ `CommandTimeout`〔30s＝OpenTouryo `FxSqlCommandTimeout`〕>⑤ `LOCK_TIMEOUT`〔-1〕。セッション/プール〔20分〕は別カテゴリ。出典＝『ASP.NETで考慮すべきタイムアウト値』＋実ソース）
                                  └ references/file-upload-download.md 実効38L tok~1385  ファイルのアップロード/ダウンロード（P層。net48＝`Request.Files`/`SaveAs`・`Response.WriteFile`/`TransmitFile`／Core＝`IFormFile`/`FileStreamResult`。制限＝`maxRequestLength`/`maxAllowedContentLength`。**セキュリティ**＝拡張子＋マジックバイト検証・ファイル名はアプリが決める・パストラバーサル・wwwroot 外・認可・日本語名 RFC6266。OpenTouryo 2ページ＋最新動向〔MS Learn/セキュリティ記事〕で裏取り）
                                  └ references/cache-control.md    実効34L tok~1204  キャッシュ制御（P層・**Web Forms/MVC 両対応**〔不正操作防止系の中で唯一 MVC でも効く例外〕。方針＝**動的/認証画面はキャッシュ無効**。OpenTouryo は config **`FxCacheControl`**〔`MyLiteral.CACHE_CONTROL`。ON で親クラス2〔Web Forms=`MyBaseController`／MVC=`MyBaseMVController`・`MyBaseMVControllerCore`〕が no-cache/no-store/Pragma/Expires 付与・既定 OFF・ON 推奨〕、D層 SQL は `FxSqlCacheSwitch`。最新＝`[ResponseCache]`/**OutputCache〔.NET7+〕**/`IMemoryCache`/`IDistributedCache`・機密は `no-store`。実ソース＋MS Learn で裏取り）
                                  └ references/illegal-operation-prevention.md 実効66L tok~2368  不正操作防止（P層・**Web Forms 専用/MVC には無い**。公式「Webアプリケーションの不正操作」の**6分類**〔①新規ウィンドウ②URL直打ち③二重送信④戻る⑤リロード⑥読込停止後〕→**C=クライアント/S=サーバ**の対策割付＋**4機能**〔不正操作防止S／二重送信防止C〔`FxDoubleTransmissionCheck=on`→`onSubmit="return Fx_OnSubmit();"` JS。`BaseController.cs` L672〕／画面遷移制御〔②URL拒否・①窓制限〕／ブラウザ・窓別Session領域〕を先頭マップに。リロードの HTTP メソッドは遷移方式依存〔`Server.Transfer`=POST再送／`Response.Redirect`=GET再送〕→PRG。二重送信/リロード/**戻る再送**/キャッシュ参照を防止＝**リクエスト チケット**〔マスタの Hidden `RequestTicketGuid` と Session 側 `Queue<string>`〔LRU 世代〕をサーバ突合。世代数＝config **`FxRequestTicketGuidMaxQueueLength`**〔`FxLiteral` L80〕〕。**検出時も Session は解放せず業務続行可**。誤検知は画面別 **`CanCheckIllegalOperation`**〔`bool?`・`BaseController` L300〕。関連＝`FxDoubleTransmissionCheck`/`FxButtonhistoryMaxQueueLength`/`FxScreeenGuidMaxQueueLength`/`FxWindowGuidMaxQueueLength`。**キャッシュ制御・画面遷移制御と三点セット**。最新＝PRG/シンクロナイザ・冪等化トークン/CSRF アンチフォージェリ〔目的が別・両方必要〕。実ソース＋公式＋MS Learn/OWASP で裏取り）
                                  └ references/list-paging.md      実効38L tok~922  一覧ページ制御方式（決めること＝最大表示/取得件数＋ページ制御方式。3方式〔①アプリ制御＝主キー先取り・ODP `FetchSize`／②ストアド範囲／③**SQL＝`ROW_NUMBER`/`TOP`/`ROWNUM`**〕。DBMS 別 SQL〔SQL Server `TOP`・`ROW_NUMBER`+CTE〔2005+〕／Oracle `ROWNUM`・`ROW_NUMBER`〕＝定義ファイルも DBMS 別。**大量は SQL でページング**〔全件取得しない〕・総件数は `COUNT`〔`D5_SelCnt`〕・ストリーミングは `ExecSelect_DR`・UI ページャとデータ取得の役割分担。出典＝公式「一覧ページ制御方式」＋query-definition/layer-d/webforms-event 連携）
                                  └ references/state-management.md 実効47L tok~1233  ASP.NET 状態管理方式（**★ ViewState・Server.Transfer＋HTTP Context は Web Forms 専用**／Session・Application・Cache は ASP.NET 全般。クライアント側〔ViewState・Hidden・Cookie・QueryString・Control State〕/サーバ側〔Session〔20分・負荷分散は StateServer/SQLServer・直列化〕・Application・Cache〔自動破棄・null チェック〕・静的変数〕をスコープ・寿命で選択。負荷分散＝`machineKey` 全ノード統一。**OpenTouryo 対応マップ**：Fx 隠しフィールド→illegal-operation/webforms-screen・Session 領域→webforms-dialog・Cache→cache-control・アプリ定数→共有情報〔`GetSharedProperty`〕・認証チケット→auth・`DataTable` 保持→batch-update。出典＝MS 技術情報「状態管理方式」＋既存スキル）
                                  └ references/concurrency-tuning.md 実効47L tok~1353  同時実行性・スレッド/接続の環境チューニング（**★ ランタイムで全く別**：net48 classic〔`machine.config` `processModel`〔maxWorker/IO Threads〕・`httpRuntime`〔minFreeThreads〕・`connectionManagement maxconnection`〔外部接続・既定2〕・`aspnet.config applicationPool`・レジストリ `MaxConcurrentRequestsPerCPU`〔既定12〕。**多くは `processModel autoConfig` で自動＝手動は稀**〕／net10.0 Core〔classic 設定は**無効**。**第一原則＝async 化**・Kestrel `Limits.MaxConcurrentConnections`〔既定 null〕・アウトバウンドは `SocketsHttpHandler.MaxConnectionsPerServer`＋`IHttpClientFactory`〕）。旧＝MS 技術情報「ASP.NET config（同時実行性）」／最新＝MS Learn/Kestrel・HttpClient を WebSearch で裏取り。先回りで machine.config を弄らず実測後に）
                                  └ references/character-encoding.md 実効49L tok~1188  文字コード・エンコーディング（内部 UTF-16／外部 UTF-8＝**UTF-8/Unicode 統一**。**OpenTouryo 部品**〔`Public/Str`〕：**`StringChecker`**〔`IsInCodePage`/`IsShift_Jis`＝可逆チェック・半角/全角/かな/漢字判定〕・**`JIS2k4Checker`**〔`CheckSurrogatesPairChar`/`DeleteSurrogatesPairChar`＝サロゲートペア=絵文字/第3・4水準の検出/除去〕。JIS 水準〔X0208/X0212/X0213・JIS2004〕。HTML SJIS→数値文字参照。DB＝`nvarchar`/MySQL `utf8mb4`。連携先＝`FxSqlEncoding`/config・BOM/comment-convention・resx/message。※ wiki の `JISX0208_1983Checker` は別クラスでなく `StringChecker.IsInCodePage`/`IsShift_Jis` 相当。実ソース＋MS Learn で裏取り）
                                  └ references/internationalization.md 実効56L tok~1461  国際化 i18n／地域化（対応項目＝文言/書式/カレンダー/タイムゾーン/和暦・元号/双方向 RTL/文字コード。.NET＝`CultureInfo`〔`CurrentCulture`=書式・`CurrentUICulture`=UI 表示を分ける〕/`.resx`/`JapaneseCalendar`/UTC 保持。**★ 推奨 C/S 設計**：UI 表示＝クライアント可変／ログ＝サーバ固定／書式＝クライアント or 統一。**OpenTouryo 対応部分**：メッセージ カルチャ〔`FxExceptionMessageCulture`/`MSGDefinition_<culture>.xml`=message〕・**`GMTMaster`**〔public・ローカル⇔UTC〕・**`FormatConverter`**〔public・丸め/桁区切り〕・`StringChecker`/`JIS2k4Checker`〔character-encoding〕。※`MyTimeZone` は internal。和暦は net48 `EnforceJapaneseEraYearRanges`/Core は別。実ソース＋MS Learn で裏取り）
                                  └ references/screen-composition.md 実効40L tok~991  画面の構成（WebForms/WinForms/MVC。**共通の考え方＝親〔デザインベース〕＋個別〔子〕＋ユーザコントロール**を、実現手段別に対比：WebForms＝**マスタページ**〔`.master`/`BaseMasterController`/`ContentPlaceHolder`/**Fx 隠しフィールド**〕・`.ascx`／WinForms＝**ベース Form**・UserControl／MVC＝**`Views/Shared/_Layout.cshtml`＋`RenderSection`/`RenderBody`**・**HTML ヘルパー**。**★ MVC の `_Layout` に Fx 隠しフィールドは無い＝標準 Razor・OpenTouryo 独自マスタ機構なし**〔不正操作防止/画面遷移は WebForms 専用〕。ボタンも WebForms=UOC 自動結線／MVC=Action。実装は各 P層スキル。OpenTouryo 公式＋MS Learn〔Razor Layout〕で裏取り）
                                  └ references/table-driven-control.md 実効39L tok~1320  親クラス2 のテーブル駆動制御（多言語化・UI 制御・**閉塞**）。**親クラス2 `UOC_CMNFormInit` 等の共通処理でテーブルを引いて制御**する纏め者パターン。**①多言語化 辞書テーブル**〔画面名/コントロール名/各言語・SQL/XML/DataTable。resx 代替＝可視性良〕**②権限・状態制御**〔ロール/状態/表示・活性→auth〕**③閉塞・閉塞チェック**〔閉塞テーブル・機能/画面/イベント単位・障害/運用閉塞。**全 P層親クラス2 の `// 権限/閉塞チェック` 空 stub** に実装〔`MyBaseController`/`MyBaseMVController(Core)`/`MyBaseAsyncApiController(Core)`/`MyBaseControllerWin`〕→閉塞なら業務/システム例外に閉塞 messageID でスロー→exception〕。**★ ①②は Control 再帰＝WebForms/WinForms のみ／③閉塞はテーブル引き＋例外で全 P層方式可**。実装は base2-customize。公式3ページ＋stub 実在で裏取り）
                                  └ references/batch-processing.md  実効61L tok~1241  バッチ処理方式（P層＝コンソール `Program.Main`〔**専用基底なし**・`GetCommandArgs` でコマンドライン解析→pv 組立→**`new LayerB().DoBusinessLogic()` 直呼び**〕。**大量データの落とし穴6点→対策**：全件メモリ→**主キー先取り**〔`SelectPkList`〕・ロングTx→**コミット間隔**〔`INTERMEDIATE_COMMIT_COUNT` ごと1Tx〕・リラン→**再開位置をファイル出力**〔`initialIndex`〕・多重化不可→**EXE多重起動＋PK範囲引数**・フェッチ→`ExecSelect_DR`・DB間通信→配列バインド/バッチSQL。`RerunnableBatch` フロー＝`Read→(処理→Write)→loop`。D層技法は batch-update/dao-generated〔`DamManagedOdp.ArrayBindCount`・`SQLUtility`・クエリキャッシュ。※公式の `DamOraOdp` は表記で実体 `DamManagedOdp`〕。オンライン起動＝同期 `WaitForExit`/非同期 `Process.Start`・排他 `Semaphore`/`Mutex`。実サンプル `Bat_sample`〔Simple/Rerunnable〕で裏取り）
opentouryo-comment-convention     実効78L tok~1953  完了（**コメント／ファイルヘッダ規則**＋**UOC 範囲**を示すインライン節区切り `↓`/`↑`。新規 `.cs`/`.vb` の先頭に所属機能名バナー＋クラス情報ブロック〔クラス名/日本語名/作成日時/作成者/更新履歴表〕を言語コメントで〔C#=`//*`・VB=`'*`・バナー=接頭辞＋`*`×82・UTF-8 BOM〕。クラス/メソッドは `/// <summary>` XML doc、インラインは言語仕様。`.aspx`/`.master` マークアップは対象外・自動生成物はツール任せ。**インライン節区切り**＝D層 `DBアクセス`・B層 `業務処理`・呼び出し側 `B層実行：〈説明〉`〔自由記述〕の3種で **UOC（User Own Coding＝スケルトンを除く、開発者が書く＝処理変更時の修正範囲）** を囲む（ダッシュはラベル後を埋めて幅を揃える＝短ラベルは `-`×53）。実サンプル `LayerB.cs`/`LayerB.vb`/`LayerD.cs`/`Bat_sample Program.cs` で裏取り）
...
opentouryo-project-setup-config   実効86L tok~2040  完了（⑥⑦。resource 移設・config パス張替〔%OT_RESOURCE_ROOT%〕・.gitignore・接続文字列/InitConfiguration/StateServer・ビルド/実行検証。references を保持）
...
opentouryo-batch-update        実効110L tok~2865  完了（**DataTable の RowState バッチ更新**。グリッド外[追加]=Added・グリッド内[削除]=`dr.Delete()`=Deleted・セル編集=Modified を `switch(dr.RowState)` で自動生成 Dao の S1_Insert/D3_Update/D4_Delete に振り分け。`DataRowVersion.Original` で楽観排他〔Deleted 行は Original のみ〕、`AcceptChanges`、Web は Session 保持、大量は SQLUtility/ExecGenerateSQL。実サンプル GenDaoAndBatUpd_sample で裏取り）
opentouryo-webforms-crud-screens 実効70L tok~2452  完了（**Web Forms テーブル保守 CRUD 画面の P層パターン**〔**単独スキル**。元は batch-update の reference→**2026-07-28 昇格**：(1) がバッチでない〔単一レコード CRUD〕ためスコープ不一致〕＋`references/snippets.md`〔コピー元コード＋**自動生成→推奨の「何処を→どう」書き換え対比表**・予算外〕。**(1)一覧→詳細＝オーソドックス・既定**／**(2)一覧＆更新＝特殊**〔UPDATE/DELETE は一覧で複数行 RowState バッチ・INSERT は詳細〕。**★ (2) は最初の編集で結果セットを固定＝ページング停止・Session の DataTable に再バインド**〔ページングは再取得で RowState が消えるため〕。ページングは P層 GridView+ObjectDataSource `EnablePaging`／D層 `ROW_NUMBER` SQL〔list-paging〕。詳細画面は Session の PK 有無で INSERT/表示モード分岐。**★ 自動生成 `_3TierEngine`/`CmnTableAdapter`/`ObjectDataSource` は推奨実装と別＝構造の参考に留め、`LayerB`/`DoBusinessLogic`＋自動生成 Dao＋DPQ＋RowState へ置き換える置換表つき**。出典＝WORKSPACE `Aspx/sample/3Tier/Products*`＋`_3TierEngine.cs`/`CmnTableAdapter.cs`）
opentouryo-query-definition    実効364L tok~4753  完了
opentouryo-message             実効129L tok~1861  完了
opentouryo-shared-property     実効 62L tok~ 777  完了
opentouryo-screen-transition   実効131L tok~2388  完了
opentouryo-transaction-control 実効115L tok~2116  完了
opentouryo-transmission        実効128L tok~2261  完了
opentouryo-exception           実効277L tok~4569  完了
opentouryo-logging             実効177L tok~2523  完了
...
**全39スキルの本文を書き終えた。** 全て標準準拠、目安（500行 / 5000トークン）内。
...
| **作者 field-test（TestPlan2 ① 実装＋実機確認）レポートの反映**（**2026-07-26**。作者の提案＋as-built OTRVCAS で裏取り） | 実装レポートから3点を反映。**(A) 環境変数の衝突回避（作者提案）**：`OT_RESOURCE_ROOT` はユーザ環境変数＝マシンで1つ。既に別プロジェクトが設定済み（実測：OTRVCAS の resource を向いていて新規 SQL 定義がそちらへ紛れた）なら奪い合わず**別名（例 `OT_RESOURCE_ROOT2`／プロジェクト固有名）**にして config 各キー＋log4net `%env{...}%` を張り替え・`SETUP-CHANGES.md` 記録→ `opentouryo-project-setup-config/references/resource-config.md`。**(B) IDENTITY 主キーの落とし穴＋(C) 楽観排他＋実行順**：作者指摘で**バッチ更新に限らない D層一般則**と判明→ `opentouryo-layer-d` へ集約。**節「複数行 INSERT/UPDATE/DELETE の注意」**：①IDENTITY は INSERT で採番 PK がメモリに戻らない→続けて更新/削除で `WHERE PK IS NULL`＝0件排他→再 SELECT、②**実行順は DELETE→INSERT（INSERT→DELETE はキー衝突）**。**さらに作者指摘で楽観排他は「混在の注意」と別テーマ→独立節「楽観排他方式」**を新設：判定は共通で「更新/削除 件数0＝業務例外」／**タイムスタンプ列あり**＝自動生成Dao に自動組込（主キー＋ts を渡し件数0チェックのみ・`opentouryo-dao-generated`）／**無し**＝`D3_Update`/`D4_Delete` の WHERE に全列 `Original`（NULL→IS NULL）で全行一致。batch-update は**要点＋layer-d 参照**に圧縮（`RowState` は Deleted→Added 順）。報告の #3/#4（編集列3つ・未使用ボタン）はプロジェクト固有でスキル変更不要。**「実測」強調の見直し**（自明な事柄は外す・必要最小限に）：`OT_RESOURCE_ROOT` 衝突（単一のマシン共有変数＝自明）と `project-setup` の「未コミットは失われうる＝実測」を平文化。発見系の実測は温存。tok：layer-d ~2870・batch-update 微減（Bash 停止中につき復帰後再測）・resource-config.md/project-setup は予算内 |
| **作者 field-test（TestPlan2 ① 再実装＋実機確認）レポートの反映**（**2026-07-26**。SQL Server 挙動・`run-verify.md` で裏取り） | 大半はスキルが効いている確認（マスタ5ボタン＋`UOC_CommonMaster_btnMainX_Click`〔接頭辞＝マスタ名〕・`FxScreenTransitionMode=off`→UOC 戻り値 URL＋`ScreenTransitionMethod=2`・バッチは Deleted→Modified→Added・件数0 業務例外・IDENTITY 再取得——いずれも既反映分が機能）。**新知見3点を反映**：**(A) `ntext`/`text`/`image`（LOB）は SQL Server で `=` 比較不可**→ 無タイムスタンプの全列 WHERE 楽観排他が LOB 列で実行時エラー。`opentouryo-layer-d` 楽観排他方式に注記（LOB 列は WHERE から除く／ts 列を足す／`S3`・`S4` に割り切る）。**(B) グリッド/テンプレート内コントロールに自動結線接頭辞（`txt`/`lbn`）を付けない**（付けると行ごとに不要イベントが自動結線）→ 接頭辞なし＋`FindControl`、グリッド自身の `RowCommand` は使わないなら空実装→ `opentouryo-layer-p-webforms-event` グリッド節に注記。**(C) 環境変数の使い分け**：検証だけなら User 変数を変えず**プロセス限定上書き**（`$env:OT_RESOURCE_ROOT`＝`run-verify.md` に既収録）で足り `SETUP-CHANGES.md` 不要／恒久運用だけ別名（`OT_RESOURCE_ROOT2`）——resource-config.md にこの使い分けを明記。報告の申し送り#2（楽観排他を主キーのみにした理由＝ntext）は (A) の裏付け。tok：layer-d 3503・webforms-event 3945（上限内・snippet/resource-config は予算外） |
| **作者指摘「パラメタ・クリア後の動的SQL再処理は系統で違う」を実ソース検証→正しいと確認・反映**（**2026-07-26**。`DamSqlSvr.PreExecQuery`／`BaseDam.init` で裏取り） | 作者仮説＝個別/共通Dao は動的SQL処理後の静的SQLにパラメタセット・自動生成Daoは動的SQL再処理。**正しい**と確認。機構：`PreExecQuery`（各 Exec 前）は **DPQ のときだけ** `Convert()` で静的SQL化〔DamSqlSvr L594〕→直後に `_QueryStatus` を **DPQ→SPQ 切替**〔L599〕→末尾 `init()`〔BaseDam L238-255：`_xml=null`・パラメタ辞書クリア・`_QueryStatus=SPQ`〕。SPQ 時の `PreExecQuery` は `else`＝何もしない〔L662〕。よって**個別/共通Dao は SQL を1回セット→初回 Exec で静的化・以後クリア＋再セット＋実行は静的SQLに値差し替えのみ（動的SQL再処理なし・ただし動的構造は初回で固定）**。**自動生成Dao は生成ラッパが毎回 `SetSqlByFile2` を呼ぶ→毎回 DPQ 再処理**（行ごとに列可変な代わりに遅い＝クエリ・キャッシュで軽減）。→ `opentouryo-layer-d` のパラメタ・クリア節に注記（クエリ・キャッシュ注記を統合）。tok：layer-d 3724（上限内） |
...
| **README で `opentouryo-exception` を ③→② へ移動**（作者指摘・**2026-07-26**） | exception は「層を問わず参照する全層常用の横断」（AGENTS.md の実装時必須ルール「業務例外はリスローされない」）だが、③「制御・定義／横断機能（必要になったとき参照）」で XML 定義系（message/shared-property/transmission）と同列＝常用性が過小に見えた。→ **②「各層のコード実装（日常常用）」の `opentouryo-layer-b` 直後へ移動**し「全層で常用する横断・層を問わず参照」を注記。③ からは除去。README のみ変更（スキル行は36で不変） |
| **README にグループ④「レビュー・フェーズ」を新設し `opentouryo-log-analysis` を切り出し**（作者指摘・**2026-07-26**） | log-analysis は「出力ログからエラー・性能の対応を提案＝実行後の分析」で、③（制御・定義／横断）とは位相が違う。→ **④「レビュー・フェーズ（実装・実行の後：ログ／エラー／性能を分析して対応を提案）」を新設**し log-analysis を移動（③ から除去）。当面1スキルだが今後のレビュー観点スキルの受け皿。`src/instructions/AGENTS.md` の README 参照文「用途別・3分類」→「用途別に分類」に（グループ増でも stale にならないよう数字を除去）。README＋AGENTS のみ変更（スキル行36で不変） |
| **新スキル `opentouryo-comment-convention`（コメント／ファイルヘッダ規則）を新設**（作者依頼・**2026-07-26**。`Samples`/`Samples4NetCore` の `.cs`＋`root/programs/VB` の `.vb` で裏取り） | 実サンプルの規則を抽出：**新規 `.cs`/`.vb` の先頭に2ブロックのファイルヘッダを言語コメントで埋め込む**——①所属機能名バナー（レイヤ P/B/D）②クラス情報ブロック（`クラス名`/`クラス日本語名`/`作成日時`/`作成者`/`更新履歴` の表）。接頭辞は **C#=`//*`／VB=`'*`**、**バナー=接頭辞＋`*`×82**、**UTF-8 BOM**（`LayerB.cs`/`LayerB.vb` で同一書式を確認）。コード内コメントは言語仕様（クラス/メソッド=`/// <summary>`〔VB=`''' <summary>`〕・インライン=`//`〔VB=`'`〕）。`.aspx`/`.master` マークアップはバナー対象外（`<%@ Page … %>` 始まり・`<!-- -->`/`<%-- --%>`）、自動生成物はツール任せ。反映：SKILL＋snippet（C#/VB 全文テンプレ・更新履歴追記例）／**AGENTS.md 実装時の必須ルールにポインタ**／README ②へ「全層・全ファイル共通」で追加／§3（**全36→37スキル**）。tok：comment-convention 1284・desc 395字・name一致 |
| **`opentouryo-comment-convention` に「インライン節区切りコメント `↓`/`↑`」を追記**（作者指摘・WORKSPACE の実装に感心・**2026-07-26**。サンプル/WORKSPACE の `LayerD.cs`/`LayerB.cs` で裏取り） | 作者が WORKSPACE で見た `// ↓DBアクセス----…`／`// ↑業務処理----…` の節区切りを再現性向上のため追加。実サンプル（MVC/WebForms/WS の `LayerD.cs`＝**`DBアクセス`**〔`SetSqlByFile2`/`SetParameter`/`ExecXxx` の一連を囲む〕・`LayerB.cs`＝**`業務処理`**〔業務ロジック本体を囲む〕）で確認。書式＝**開始 `↓`／終了 `↑`＋ラベル＋ダッシュ**（ラベル後を埋めて幅を揃える＝短ラベルは `-`×53〔`LayerD.cs` で実測〕）、コードのインデントに合わせる。**ラベル3種**：D層 `DBアクセス`・B層 `業務処理`・呼び出し側（P層/バッチ `Program`）`B層実行：〈説明〉`〔`：`後に自由記述。`Bat_sample/Program.cs` で確認〕。SKILL にレイヤ×ラベル表＋snippet に D層/B層/呼び出し側の実例3つ。tok：comment-convention 1284→1648（上限内）。※網羅探索で第3ラベルを確認して追補 |
| **`↓`/`↑` 節区切りの意味を「UOC（User Own Coding）の範囲」に是正**（作者指摘・**2026-07-26**） | 当初「メソッド内の論理的なまとまり」と把握していたが、作者より**厳密には UOC（User Own Coding）＝スケルトン（テンプレート）コードを除く、開発者が自分で書く部分を囲む＝処理変更時の修正範囲を示す**と訂正。`UOC_` メソッド名の由来とも整合。SKILL の節見出し・導入を「UOC の範囲」に、表の列を「囲む対象」→「UOC で書く内容」に、`↓`〜`↑` の外はスケルトン＝そのまま を明記。description に UOC/節区切り/修正範囲 のトリガ語を追加。snippet 導入も UOC に。tok：comment-convention 1648→1825・desc 498字（上限内） |
| **新スキル `opentouryo-app-design`（設計フェーズの地図＝親スキル）を新設**（作者依頼・**2026-07-26**。公式「アプリケーション設計のポイント」を WebFetch し評価） | 公式ページは**フレームワーク全体の設計トピック目次**で、≒8割は既存スキル＋README/AGENTS と重複。**目次のスキル化は不要**と評価し、代わりに**設計段（spec→plan）の地図/チェックリスト**を新設：決める事項（レイヤ/例外/Tx/データアクセス/共有情報/画面/セッション・セキュリティ/認証/ログ/国際化/呼出・非同期/コメント規約）を各実装スキルへ割り付け、処理方式（画面/バッチ/非同期/組込/ワークフロー）選択を先頭に。**未整備領域**（帳票・印刷・メール／バッチ処理方式／WebAPI／性能・高信頼性／組込・ワークフロー）を明示し、**作者が個別課題を提示 → 子スキル/references/snippets で順次整備**する親スキルとして位置づけ。反映：SKILL のみ（設計の地図＝コード無しなので snippet なし）／README ②先頭に「設計フェーズの地図」＋凡例にファサード追記／AGENTS.md「開発の進め方」step2 に漏れ防止ポインタ／§3（**全37→38スキル**）。tok：app-design 1681・desc 400字・name一致 |
| **`opentouryo-app-design` の最初の設計課題「タイムアウト値」を references として整備**（作者提示 URL・**2026-07-26**。`PubLiteral.SQL_COMMANDTIMEOUT` で裏取り） | 公式「ASP.NETで考慮すべきタイムアウト値」を WebFetch。設計チェックリスト＝実装コード無しの指針なので**子スキルでなく `references/timeout-values.md`** で整備。原則＝**呼び出し元（外側）ほど長く**（内側=下流が先に切れて外側が失敗を受け取る）。リクエスト経路のネスト〔① HTTP `HttpClient`/`WebRequest` 100s ＞② `executionTimeout` 110s ＞③ `ConnectionTimeout` 15s／④ `CommandTimeout` 30s ＞⑤ `LOCK_TIMEOUT` -1〕と、別カテゴリのセッション/プール〔20分・プール≧セッション〕を表に。OpenTouryo 連携：④＝config **`FxSqlCommandTimeout`**〔`PubLiteral.SQL_COMMANDTIMEOUT="FxSqlCommandTimeout"` L61 で確認〕、①＝`OAuth2AndOIDCClient.HttpClient`、セッション＝`auth`/`config`。**ついでに dao-common の表記を実キー `FxSqlCommandTimeout` に是正**（従来 `SQL_COMMANDTIMEOUT`＝C# 定数名を config キーと誤記）。app-design 本文にチェックリスト行＋references ポインタ。tok：app-design 1681→1798・dao-common 2303→2338・timeout-values.md 883（予算外） |
| **`opentouryo-app-design` に設計課題「ファイルのアップロード/ダウンロード」を references として整備**（作者提示 2URL＋最新動向収集の指示・**2026-07-26**） | OpenTouryo 2ページ（アップロード/ダウンロード＝net48/classic の基本）を WebFetch＋**最新動向を WebSearch**（ASP.NET Core・セキュリティ＝MS Learn/C# Corner）。**`references/file-upload-download.md`** に整備。**アップロード**：net48 `Request.Files`/`HttpPostedFile`/`SaveAs`・`maxRequestLength`〔KB既定4MB〕/`maxAllowedContentLength`〔既定約28.6MB〕／Core `IFormFile`（小）＋アンバッファ ストリーミング（大）・`MultipartBodyLengthLimit`。**ダウンロード**：net48 `Response.Clear/WriteFile/BinaryWrite/TransmitFile〔大容量・カーネル〕/End`・`Content-Disposition`／Core `File()`/`PhysicalFileResult`/`FileStreamResult`〔ヘッダ/エンコード/Range を FW 処理〕。**日本語名＝RFC6266/5987 `filename*=UTF-8''`**。**セキュリティ鉄則**（最新）：サイズ制限・拡張子許可リスト＋Content-Type＋**マジックバイトで中身検証**・**ファイル名はアプリが決める**（パストラバーサル）・**wwwroot 外/実行権限外し**・**認証/認可**・ウイルススキャン・`nosniff`。P層は `layer-p-webforms-event`/`-mvc`、制限は `config`、認証は `auth` へ相互参照。app-design にチェックリスト行「ファイル入出力」＋references ポインタ。tok：app-design 1798→1924・file-upload-download.md 1385（予算外） |
| **`opentouryo-app-design` に設計課題「キャッシュ制御」を references として整備**（作者提示 URL・**2026-07-27**。`MyBaseController`/`MyLiteral`/`PubLiteral` で裏取り＋最新動向 WebSearch） | OpenTouryo「キャッシュ・コントロール」を WebFetch。方針＝**動的/認証画面のレスポンスはキャッシュ無効**（戻るボタン/プロキシ残存/機密漏えい対策）。実ソース裏取り：スイッチ config **`FxCacheControl`**〔`MyLiteral.CACHE_CONTROL="FxCacheControl"`。`ON` で `MyBaseController` が `Cache-Control: no-cache,no-store,must-revalidate`＋`Pragma: no-cache`＋`Expires: 0` を付与。**既定 OFF**＝方針として ON 推奨〕、D層 SQL キャッシュは別物 **`FxSqlCacheSwitch`**〔`PubLiteral.SQL_CACHE_SWITCH`〕。ファイル DL で上書きは `Response.Clear` 再設定（download reference と直結）。不正操作防止/画面遷移制御と併用。最新動向〔MS Learn/`aspnetcore-10.0`〕：`[ResponseCache]`（ヘッダのみ）／**OutputCache ミドルウェア〔.NET7+・UI 推奨・既定 MemoryCache〕**／`IMemoryCache`（単一）・`IDistributedCache`（複数・Redis）／静的は max-age+immutable+ハッシュ／**認証依存はキャッシュしない＝OpenTouryo と同結論**。→ `references/cache-control.md`＋app-design にチェックリスト行「キャッシュ制御」＋references ポインタ。tok：app-design 1924→2016・cache-control.md 1079（予算外） |
| **`opentouryo-app-design` に設計課題「不正操作防止機能」を references として整備**（作者提示 URL・**2026-07-27**。`BaseController.cs`/`FxLiteral.cs` で裏取り＋最新動向 WebSearch。「キャッシュと関係が深い」との指摘） | OpenTouryo「不正操作防止機能」を WebFetch。対象＝P層。**二重送信/リロード/戻る再送/キャッシュ参照**（＝同一ユーザによる送信済みフォームの再現）を防ぐ＝**リクエスト チケット**方式。実ソース裏取り：**マスタの Hidden `RequestTicketGuid`**〔`FxLiteral.HIDDEN_REQUEST_TICKET_GUID="RequestTicketGuid"` L443・`FindControl` L576〕と **Session 側 `RequestTicketGuid_Queue`（`Queue<string>`・LRU 世代）**〔`BaseController.cs` L194/L197〕をサーバ突合。世代数＝config **`FxRequestTicketGuidMaxQueueLength`**〔`FxLiteral` L80。戻り許容段数の目安〕。**検出時も Session は解放せず＝業務続行可能**（致命でなくやり直し可）。誤検知は画面別 **`CanCheckIllegalOperation`**〔`bool?`・`BaseController.cs` L300〕で調整。関連スイッチ＝`FxDoubleTransmissionCheck`/`FxButtonhistoryMaxQueueLength`/`FxScreeenGuidMaxQueueLength`/`FxWindowGuidMaxQueueLength`（config/webforms-dialog へ相互参照）。**キャッシュ制御（`FxCacheControl`）・画面遷移制御と三点セット**（キャッシュ古画面から戻る/再送で Ticket 不整合→検出）。最新動向〔MS Learn/OWASP〕：**PRG**（偶発二重送信）／シンクロナイザ・**冪等化トークン**（Request Ticket と同型・CSRF も Session に上限付き複数世代保持の強化策あり＝同発想）／**CSRF アンチフォージェリ**〔★目的が別＝別サイトからの強制送信 vs 同一ユーザ再送。両方必要〕／API 冪等化キー。→ `references/illegal-operation-prevention.md`＋app-design「セッション/セキュリティ」行を references ポインタ付きに拡充・references ポインタ行に追加。tok：app-design 2016→2097・illegal-operation-prevention.md 1366（予算外） |
| **`illegal-operation-prevention.md` に上位マップ「Webアプリケーションの不正操作」（キャッシュ⇄不正操作防止の中間情報）を追加**（作者提示 URL・**2026-07-27**。`BaseController.cs` L658-691 で二重送信防止＝JS を裏取り） | 公式「Webアプリケーションの不正操作」を WebFetch。**不正操作を6分類**〔①新規ウィンドウ〔Session 競合〕②URL 直打ち〔Form/Session 不足→NullReference〕③二重送信④戻る（バック・サブミット）⑤更新（リロード）⑥読み込み停止後の操作〕し、各々を **C=クライアント側制御（JS・クロスブラウザ限界）／S=サーバ側検出** に割り付ける表を reference 先頭に追加。**4機能の役割分担**を明確化：不正操作防止機能〔S・Request Ticket。④⑤⑥＝送信済み画面の再現を一括検出〕／**二重送信防止機能〔C〕**〔裏取り：`FxDoubleTransmissionCheck=on`→`this.Form.Attributes.Add("onSubmit","return Fx_OnSubmit();")`＝JS 仕込み。`BaseController.cs` L672-676〕／画面遷移制御機能〔②URL 直打ちの GET 拒否・①ウィンドウ制限〕／ブラウザ・ウィンドウ別 Session 領域〔①競合回避〕。**④はキャッシュ絡み→キャッシュ無効化（`FxCacheControl`）と一体**を明記。**リロード時 HTTP メソッドは遷移方式依存**〔`Server.Transfer`後=POST 再送／`Response.Redirect`後=GET 再送〕→この非対称で PRG が成立、と補足（`opentouryo-screen-transition`）。既存の Request Ticket 詳細・関連 config 表・最新動向はそのまま。tok：illegal-operation-prevention.md 1366→2150（予算外） |
| **横断的な設計トピックの配置方針を明文化**（作者指示・**2026-07-27**） | 「単一の実装スキル ホームを持たない横断トピックは単体スキル化せず `opentouryo-app-design/references/` に置く」を方針として確定し **§2.8 に記録**。契機＝`illegal-operation-prevention.md` の配置が適切かの問い。裏取りで「Web アプリの不正操作」4機能のうち単体スキルは画面遷移制御のみ・残り3つは `opentouryo-config` のスイッチと判明 → 不正操作防止だけの単体スキル化は非対称ゆえ見送り、references 据え置きが適切と判断。昇格条件（UOC カスタマイズ スニペットが必要になったら「Web セキュリティ実装」として一括昇格）も §2.8 に明記。ドキュメントのみ変更（スキル本文・トークン不変） |
| **公式「二重送信防止機能」「ブラウザ・ウィンドウ別Session領域」2ページ照合**（作者提示 URL・**2026-07-27**。`BaseController.cs` で裏取り） | 不正操作防止クラスタの残り2機能の専用ページを WebFetch し既存記述と突合。**二重送信防止**＝既記述（`FxDoubleTransmissionCheck=on`→`onSubmit="return Fx_OnSubmit();"` のクライアント JS／一時無効化 `Form.Attributes.Remove("onSubmit")`）は正確・追加不要（新出は `document.readyState` 判定＋「Loading…」ダイアログ＝微細で見送り）。**ブラウザ・ウィンドウ別Session領域**＝**ドキュメントのメソッド名 `SetDataToBrowserWindowSession` 等は表記ゆれで、実ソースは "Session" 無しの `SetDataToBrowserWindow`/`GetDataFromBrowserWindow`/`DeleteDataFromBrowserWindow`**〔`BaseController.cs` L3195/3205/3215/3224〕→ 既存スキル（webforms-dialog・§4.3）が実ソースどおりで正しいので **wiki に合わせず現状維持**。**有用な新情報＝Session 領域は入れ子2層**：外側＝ブラウザ・ウィンドウ別（config `FxWindowGuidMaxQueueLength`）／内側＝親画面別（`FxScreeenGuidMaxQueueLength`。モーダルの保持先）。GET 要求（遷移リダイレクト除く）で GUID 採番・Hidden／QueryString で持ち回り・LRU 削除。→ webforms-dialog の「ウィンドウ別セッション領域」節に入れ子2層＋config キー対応＋GUID 採番を補強、illegal-operation-prevention.md の関連機能表もキー↔スコープ対応（窓別=Window/親画面別=Screeen）に精緻化。tok：webforms-dialog 2470→2601・illegal-operation-prevention.md 2150→2173（予算外） |
| **P層セキュリティ/Session 機能の「Web Forms 専用・MVC 非対応」を明文化**（作者指摘・**2026-07-28**。P層3系のベースコントローラ全確認） | 「Session 管理・不正操作防止など多くの P層機能は Web Forms のみ・MVC に無い」との指摘を裏取り。**`RequestTicketGuid`・`DOUBLE_TRANSMISSION`・不正操作・`BrowserWindow` は `BaseController.cs`（Web Forms）のみに実在**、`BaseMVController(Core)` には無い。MVC 親クラス2 `MyBaseMVController` L450-456 は「`IllegalOperationCheckError`/`ScreenControlCheckError` ならセッション解放しない」**防御ハンドリングの残置のみ＝検出機構は持たない**。**★ 重要な例外＝キャッシュ制御 `FxCacheControl` は MVC でも実装あり**（`MyBaseMVController` L594・`MyBaseMVControllerCore` L275 が同じく no-cache 付与）＝「Web Forms 専用」に含めてはいけない。反映：①`illegal-operation-prevention.md` 冒頭に「★ Web Forms 専用・MVC には無い（キャッシュ制御だけ例外）」の警告ブロック②`cache-control.md` を「対象＝Web Forms・MVC 両方」に是正し親クラス2 を Web Forms/MVC 両表記に③app-design のチェックリスト「セッション/セキュリティ」に「Web Forms 専用」・「キャッシュ制御」に「両対応」を追記④**`layer-p-mvc` に新節「MVC に無い Web Forms 専用のP層機能」**（MVC は CSRF=アンチフォージェリ・二重送信=PRG/冪等化で自前対応／キャッシュ制御のみ効く）。tok：layer-p-mvc 3748→3959・app-design 2097→2123・cache-control.md 1079→1204・illegal-operation-prevention.md 2173→2368（references は予算外） |
| **作者 field-test（TestCase① 実装＋検証実行）レポート A-1〜A-11 を反映**（**2026-07-28**。全点を実ソース／生成物／`OpenTouryo.Framework.dll` に当てて裏取り。境界系の指摘が中心） | 11指摘を裏取りし既出/真ギャップを区別。**A-1〔最重大・スキル間矛盾〕**＝配布 AGENTS.md「P→B は `CallController.Invoke`・直接 `new` するな」↔ p-call-business 本文は `DoBusinessLogic(pv, iso)` 中心。**`CallController.Invoke`/`InvokeAsync` は `(serviceName, object)` の2引数のみ＝分離レベルを渡せない**〔`CallController.cs` L325/340 で確認〕、一方 `DoBusinessLogic(pv, iso)` は iso 可〔`BaseLogic.cs` L186〕。→ p-call-business に**新節「呼び出し経路の選択」**（Invoke=疎結合・WS切替・iso 不可／直呼び=iso 可・密結合、のトレードオフ表）、4ステップ②を両経路併記、AGENTS.md L132 を「既定 Invoke＋per-call iso のときだけ直呼び」に緩和、transmission に「Invoke に iso 無し」注記。**A-2**＝**msbuild は `.aspx`/`.master` を非コンパイル**（`.cs` のみ）→ `aspnet_compiler.exe -v/-p/-f` 検証を webforms-screen ★節と setup-config に。**A-3**＝**UTF-8 BOM は体裁でなく機能要件**（BOM 無しを `csc` が Shift-JIS 解釈＝日本語コメント文字化け、エージェント書込は既定 BOM 無し）→ comment-convention に理由＋後付け手順、webforms-screen に ★相互参照。**A-4**＝Web グリッド↔DataRow の index ずれ（`Deleted` は DefaultView から外れる／追加行は PK=`DBNull` で `DataKeyNames` 不成立）→ batch-update 新節＋event の `DataKeyNames` に例外注記＋snippet に読み戻しヘルパ。**A-5**＝セル編集は自動で DataTable に入らない・素朴な読み戻しは `DBNull`↔`""` で無駄 Modified 量産→ batch-update に読み戻し＋DBNull 注意。**A-6**＝**`S1_Insert` は全列必須**〔生成 `DaoCategories_S1_Insert.sql` で確認〕・一覧が全列でないなら `D1_Insert`→ batch-update。**A-7**＝**D3/D4 の全列 WHERE は「全列取得」が前提**〔生成 `DaoCategories_D3_Update.xml` の `<IF>=@col<ELSE>IS NULL</ELSE></IF>` で確認＝未設定列は IS NULL で必ず0件〕→ layer-d 楽観排他節。**A-8**＝YES/NO 後処理は**別ポストバック**で走る＝状態はダイアログを出す前に確定→ webforms-dialog。**A-9**＝確認不能（親クラス2 バイナリ・質問相手不在の自動実行）時の**安全既定＝完成文でメッセージ定義**（`%1/%2` 非依存）→ message＋project-policy に「実装非依存の安全側に倒す」フォールバック一般則（「既定値を仕様と決めつける」禁止則とは別）。**A-10**＝webforms-screen「マスタ上コントロールは designer 不要」を「コンテンツ側 designer には不要／マスタ自身の designer には必要」に是正／**マスタ上ボタンの UOC が無ければ無視＝例外にならない**〔`BaseController.cs` L2441 `CheckTypeOfMethodByName` ガード＋`InvokeMethod_NoErr`〕→ event に「未使用ボタンの空実装は不要」。**A-11**（良かった点＝screen-transition Mode/Check・batch-update `dr.Delete()`・dao-common `SetSqlByFile2` 静かな失敗・event グリッド内接頭辞なし）は維持。**B〔スキル外参照＝取り込み候補〕は A-1/A-6/A-7 に集約済み**、残り（生成 Dao の `PK_`/`ClearParametersFromHt`・`sampleScreen.master` 全体像・`WebCustomLabel:Label`・StateServer 直列化）は既存スキルで対応可のためポインタ止まり（StateServer 直列化のみ batch-update Web 節に1行追加）。tok（全て上限内）：p-call-business 2768→3169・transmission 2046→2144・webforms-screen 2946→3256・setup-config 1902→2040・comment-convention 1825→1953・batch-update 2213→2631・webforms-event 3945→4170・layer-d 3724→3857・webforms-dialog 2601→2765・message 1747→1861・project-policy 4235→4443・配布 AGENTS.md 3895→3972（snippets は予算外）。**総括：単一スキルの精度は高く、効きが悪いのはスキル境界（A-1/A-4）＝「片方だけ読むと正しく両方読むと矛盾」型。エージェント固有要件（A-2/A-3）と確認不能フォールバック（A-9）を明文化。** |
| **A-1 の枠組みを作者訂正で是正（P→B 既定は処理方式で違う／`Invoke` の iso 無しは設計）**（作者指摘・**2026-07-28**。`CallController.cs`/`FxController.cs`/`WCFTCPSvcForFx.cs` で裏取り） | 前回 A-1 で「既定＝`CallController.Invoke`」と一般化したのは誤り。作者訂正：**P→B の既定は処理方式で違う**——**Web（WebForms・MVC）＝`LayerB.DoBusinessLogic(pv, iso)` 直呼び**（インプロセス・per-call で iso 指定可）／**3層C/S（リッチクライアント・WS）＝`CallController.Invoke(論理名, pv)`**。**`Invoke` に iso 引数が無いのは制約でなく設計**——3層C/S ではP層(client)とB層(server)が分離し**分離レベルはサーバ側の関心事**。裏取り：**WS/リモートの全入口が B層を `iso=User` で呼ぶ**〔`CallController.cs` L366・`FxController.cs` L204・`WCFTCPSvcForFx.cs` L157〕→ 実際の分離レベルはサーバ側で決定（親クラス2 `MyFcBaseLogic.UOC_ConnectionOpen` の `User` 振替＝`opentouryo-project-policy`／`TCDefinition` パターン・`MyAttribute` 属性ベース＝`opentouryo-transaction-control` に既収録）。反映：p-call-business「呼び出し経路の選択」を〔経路×既定処理方式×iso×WS切替〕表に組み替え＋②を「Web=直呼び／3層C/S=Invoke」に、配布 AGENTS.md「層間の呼び出し規約」の「層を直接 `new` するな・既定 Invoke」を**処理方式別の既定**へ是正（Web は直呼びが既定）、transmission の iso 注記を「設計＝サーバ側で決定・`iso=User`」へ。tok：p-call-business 3169→3309・transmission 2144→2261・配布 AGENTS.md 3972→3939（行数 234→230 に微減）。教訓：AGENTS.md の横断規約は処理方式で分岐する既定を一般化しない |
| **3層構成（通信制御）時の実装配置ルールを追加**（作者指摘・**2026-07-28**。`Samples/WS_sample` 実物で裏取り） | 通信制御で3層を採るなら**後の物理分離に備えて実装先を分ける**：**B層・D層は `WSServer_sample` 側／型情報（引数・戻り値クラス）は `WSIFType_sample` 側**（client/server 両方が参照する共有契約＝別アセンブリ・`[Serializable]`）。裏取り：`WSIFType_sample/{TestParameterValue,TestReturnValue}.cs`（型）、`WSServer_sample/{Business/LayerB.cs,Dao/LayerD.cs}`（B/D）、使い分けも `sampleScreen_cc.aspx.cs`＝`using WSIFType_sample;`（3層）↔ `sampleScreen.aspx.cs`＝`using MyType;`（2層＝型も B/D もアプリ内）で確認。→ p-call-business「呼び出し経路の選択」に配置ルールを1項追加（setup 側の WS 取り出し・Build 配置は `opentouryo-project-setup` の `samples/webservices.md` に既収録＝役割分担）。tok：p-call-business 3309→3525（上限内） |
| **3層/2層 配置例を SKILL 本文のパス列挙から snippet へ移し自己完結化＋類似パス依存を監査**（作者指摘「サンプルは削除されうる」・**2026-07-28**） | サンプルは 2層化・整理で削除されうるので、SKILL が**サンプル ファイルのパスを「見る/写す対象」として指す**のは脆い。→ p-call-business の「実サンプル：`sampleScreen_cc.aspx.cs`＋…」を **`references/snippets.md`「3層構成での実装配置」**（3層/2層 の対比表＋`using WSIFType_sample;`/`using MyType;` 込みの自己完結コード。「サンプルは削除されうるのでこのパターンを正とする」注記）へ移し、本文はスニペット参照に。**類似監査**：コーディング系で「見る/写す」用にパスを指すのは本件のみ（修正済み）。auth `login.aspx.cs`・oauth2 `OAuth2…aspx.cs`・webforms-screen `sampleScreen` は既に「サンプル固有名＝自プロジェクトに読み替える/残さない」と明記済み。project-setup-core/-selection/-setup/-transform のサンプル名参照は**サンプル操作が仕事＝必然**でスニペット化対象外。tok：p-call-business 3525→3510（本文短縮）・snippets.md ~1544（予算外） |
| **`opentouryo-app-design` に設計課題「一覧ページ制御方式」「ASP.NET 状態管理方式」を references として整備**（作者提示 2URL・**2026-07-28**。既存スキルと突合） | **2トピック＝2 reference**（1トピック1ファイルの既存パターン）。**`list-paging.md`**：SQL レベルのページング（`ROW_NUMBER`/`TOP`/`ROWNUM`）は未収録＝真ギャップ。決めること＝最大表示/取得件数＋方式（①アプリ制御〔主キー先取り・ODP `FetchSize`〕/②ストアド範囲/③SQL 制御）、DBMS 別 SQL（定義ファイルも DBMS 別＝query-definition）、**大量は SQL でページング**（UI ページャと役割分担）・総件数 `COUNT`（`D5_SelCnt`）・`ExecSelect_DR` ストリーミング。**`state-management.md`**：作者注記どおり **ViewState・Server.Transfer＋HTTP Context は Web Forms 専用**／Session・Application・Cache は ASP.NET 全般、を明示（先の「Web Forms 専用」テーマと整合）。クライアント側/サーバ側を**スコープ・寿命で選択**、負荷分散＝`machineKey` 統一、**OpenTouryo 対応マップ**（Fx 隠しフィールド／Session 領域＝webforms-dialog／Cache＝cache-control／アプリ定数＝共有情報〔`SPDefinition` はユーザ状態でなく定数と整理〕／認証チケット＝auth／`DataTable` 保持＝batch-update）。→ app-design にチェックリスト行「一覧ページ制御」新設・「共有情報・設定」を「共有情報・設定・状態管理」に拡張＋references ポインタ2件。tok：app-design 2123→2324・list-paging.md 922・state-management.md 1233（references は予算外） |
| **WORKSPACE の自動生成 CRUD サンプルから「Web Forms テーブル保守 CRUD 画面パターン」をスキル化**（作者依頼・**2026-07-28**。`WORKSPACE/.../Products*` 実物＋`_3TierEngine.cs`/`CmnTableAdapter.cs` で裏取り） | 作者提示の `Aspx/sample/3Tier/Products{ConditionalSearch,Detail,SearchAndUpdate}.aspx(.cs)`＋`ProductsTableAdapter.cs` を精読。**2方式を抽出**：**(1)一覧→詳細**＝オーソドックス・**既定**（検索一覧で行選択→PK＋TS を Session→詳細画面で単一 CRUD／詳細は Session PK 有無で INSERT/表示モード分岐）／**(2)一覧＆更新**＝特殊（UPDATE/DELETE は一覧で複数行＝RowState バッチ・INSERT は詳細）。**★ (2) の核心＝最初の編集で「結果セットを固定」**（`RowCommand` で読み戻し/`dr.Delete()`→`AllowPaging=false`＋Session の `DataTable` に再バインド。**ページングは再取得で `RowState` が消えるため**）＝先の A-4/A-5 グリッド↔DataRow・読み戻しの実例。ページングは P層 GridView+ObjectDataSource `EnablePaging`（`CmnTableAdapter` の `SelectMethod`/`SelectCountMethod`）／D層 **`ROW_NUMBER() OVER(...) BETWEEN`**〔`_3TierEngine.cs` L78-96・DBMS 別・list-paging と一致〕。**★ 作者方針＝自動生成そのままを写さない**：`_3TierEngine`（汎用エンジン・`TableName`＋`Dictionary`＋`actionType`）/`CmnTableAdapter`/`ObjectDataSource` は framework の自動生成インフラで推奨実装と別 → **構造の参考に留め、`LayerB`/`DoBusinessLogic`〔Web 直呼び〕＋業務 ParameterValue＋自動生成 Dao（S/D・`D5_SelCnt`・ページング）＋DPQ＋RowState へ置き換える置換表**を明記。→ **`opentouryo-batch-update/references/webforms-crud-screens.md`** 新設、batch-update SKILL に 🖥 ポインタ、app-design「画面設計」行に「テーブル保守 CRUD」＋参照追加。サンプル削除に備え「パターンを正とする」注記。tok：batch-update 2631→2699・app-design 2324→2374・webforms-crud-screens.md 1781（予算外） |
| **↑を単独スキル `opentouryo-webforms-crud-screens` へ昇格**（作者指摘＋作者が commit→`git mv` 実施・**2026-07-28**） | 作者指摘：reference の **(1)一覧→詳細は単一レコード CRUD でバッチでない** → `opentouryo-batch-update` の子（reference）はスコープ不一致。妥当と判断し**単独スキルへ昇格**（screen-transition が単独スキルなのと同型＝webforms-screen/event・batch-update・screen-transition・list-paging・dao-generated を横断する実装パターン）。**命名は作者選択で `opentouryo-` プレフィクス統一**（当初 `webforms-crud-screens` を `opentouryo-webforms-crud-screens` に再 mv）。作業：作者が `git mv` reference→`SKILL.md`／エージェントがフロントマター付与（name一致・desc 646字）・導入を独立スキル化・batch-update の 🖥 ポインタと app-design 参照を skill 名へ張替・README ②に1行追加・§3 の └ サブ行をトップレベル スキル行へ昇格・**全38→39スキル**（本文・833 の件数も更新）。tok：webforms-crud-screens 1781→2269（フロントマター分。上限内） |
| **`opentouryo-webforms-crud-screens` に `references/snippets.md`（コピー元＋書き換え解説）を新設**（作者依頼・**2026-07-28**。3画面＋TableAdapter 実物から起こし・framework `_3TierEngine.cs`/`CmnTableAdapter.cs` 裏取り） | 作者依頼「出典（自動生成サンプル）を snippet 化し、何処をどう書き換えるか解説を付ける」。サンプルは 2層化・整理で削除されうるので**パスでなくコード実体を取り込む**。構成：①一覧＝行選択→PK+TS を Session→詳細遷移（`SelectedIndexChanging`・btnInsert は PK 消去）②詳細＝Session PK 有無でモード分岐＋**P→B 呼び出しの自動生成 vs 推奨の before/after**（`new _3TierEngine()`＋Dictionary → 業務 `LayerB`/`ProductsParameterValue`＋自動生成 Dao `S3_Update`/件数0楽観排他）③一覧＆更新＝`RowCommand` の読み戻し/`dr.Delete()`→**結果セット固定**（`AllowPaging=false`＋Session DataTable 再バインド）④ページング＝ObjectDataSource+`CmnTableAdapter` の `SelectMethod`/`SelectCountMethod`（★呼び先を engine→LayerB に）／D層 ROW_NUMBER。締めに**「何処を→どう」書き換えチェック表**（`_3TierEngine`→`LayerB`／`Dictionary`→業務型／engine SQL→自動生成 Dao／条件→DPQ／バッチ→RowState／そのまま＝UI 構造）。SKILL に 📋 導線追加。tok：SKILL 2269→2336・snippets.md 2786（予算外） |
| **Web バッチ更新の件数・ページング注意を明記**（作者依頼・**2026-07-28**） | 「Web 画面でバッチ更新するなら `DataTable` を Session に持つ＝**レコード件数に上限を設けるか、ページングを前提にする**。ページング時は**編集開始後にページングを止める**（再取得で `RowState` が消えるため）」を記載。反映先：**`opentouryo-batch-update` の Web 節**（本文＝Session メモリ圧迫→件数上限/ページング前提→編集開始後は停止、`webforms-crud-screens` へ相互参照）＋**`opentouryo-webforms-crud-screens` のページング節**（(2) の設計判断として簡潔に。既存の「結果セット固定」と接続）。tok：batch-update 2699→2865・webforms-crud-screens 2336→2452（上限内） |
| **DPQ `<IF>/<ELSE>` の「未設定 vs null」を是正（skill 自体の誤りが field-test 誤解の元）**（作者の問い・**2026-07-28**。`BaseDam.ProcessIFTag` L1220-1385 で裏取り） | 作者の問い「`ntext` 列 `HomePage` はパラメタを設定しなければ WHERE から消えるのでは？（DaoSuppliers_D3_Update.xml の編集は不要では）」＝**正しい**。実ソース確定：`<IF>AND [c]=@c<ELSE>AND [c] IS NULL</ELSE></IF>` は **①値設定→IF（`=@c`）②`null` 設定→ELSE（`IS NULL`）③未設定（`SetParameter` 呼ばない）→ブロックごと削除**（L1382-1384）。**query-definition L158「未設定なら `IS NULL`」は誤り**（同スキル L135「未設定なら消える」と矛盾）＝この記述が field-test エージェントの「設定しなければ 0 件」誤解の元。是正：query-definition の IF/ELSE を**3分岐の表**に＋「未設定と null を混同しない」、「テキスト内/タグ内」節の conflate 記述（L174/L178）も是正。**layer-d の A-7 note「パラメタを渡さない列は IS NULL に落ち 0 件」も同じ誤り→是正**：判定に含めるかは「その列の `@` を設定するか」で決まる／**`ntext` 等は当該列のパラメタを設定しないだけで WHERE から外れる＝生成 XML 編集不要**（値設定は `=` 比較で SQL エラー）。他スキルに同型の誤りが無いことを grep 確認。tok：query-definition 4504→4733・layer-d 3857→3945（上限内） |
| **↑の作者フォロー編集をレビューし「タグ内パラメタ `null`＝ブロック削除」を是正**（作者編集のレビュー・**2026-07-28**。`BaseDam.ProcessIFTag` L1247/L1276 で裏取り） | 作者が query-definition の「テキスト内/タグ内」表を明示化（タグ内＝`true`＝有効／`false`＝ELSE／`null`＝ブロック削除）。true/false は正だが**`null`＝ブロック削除は誤り**：`obj==null` は L1247 でテキスト内・タグ内**共通で ELSE 有効**（ELSE 無ければ L1276-1280 でエラー）＝**「ブロック削除」は未設定のときだけ**（L1378）。同スキル L186「タグ内で `false`/`null` は ELSE／無ければエラー」・L358「`null` はタグ無効化＝ELSE」とも矛盾。→ タグ内列を **`true`＝有効／`false`・`null`＝ELSE（無ければエラー）／未設定＝ブロック削除** に是正（テキスト内列と対称）。残りの節は正確で改変不要。tok：query-definition 4733→4753（上限内） |
| **公式「ログ出力方式」3ページ（総論／log4net／NLog）を照合し方向性のみ追記**（作者提示 3URL・**2026-07-28**。`LogIF.cs` で裏取り） | 3ページとも概要中心で**大半は `opentouryo-logging` に既収録**（LogIF API・ロガー名 ACCESS/SQLTRACE/OPERATION・3種トレースログ・`LogLib` 切替・`FxLog4NetConfFile`・log4net/NLog テンプレ・`%env{}`/`${}` 環境変数展開）。**config キーは実ソースで `"LogLib"`**〔`LogIF.cs` L53〕＝skill が正しく、**wiki の小文字 `logLib` は表記ゆれ（合わせない）**。`logLib.ToLower()=="nlog"`〔L63〕も記述どおり。**真のギャップ＝方向性のみ**：log4net は本家開発休止で将来廃止予定・**NLog が推奨移行先**（`LogIF` API 共通なので業務コード無変更、切替は config＋設定ファイルのみ）→ logging の LogLib 節に「新規は `LogLib=nlog` 既定」を追記。tok：logging 2404→2523（上限内） |
| **公式「ASP.NET config（同時実行性）」＋最新調査を `concurrency-tuning.md` として整備**（作者提示 URL・**2026-07-28**。旧＝MS 技術情報／最新＝MS Learn を WebSearch） | ページは **.NET Framework 時代の同時実行性チューニング**（`machine.config` `processModel`〔maxWorker/IO Threads〕・`httpRuntime`〔minFreeThreads/`executionTimeout`〕・`connectionManagement maxconnection`〔外部接続・既定2〕、`aspnet.config applicationPool`〔maxConcurrentRequestsPerCPU 等〕、レジストリ `MaxConcurrentRequestsPerCPU`〔既定12〕）。**作者注記どおり古い**ので最新調査：**classic の多くは `processModel autoConfig` で自動＝手動は稀／Core（net10.0・Kestrel）では classic 設定は無効**——**第一原則＝async 化**、Kestrel `Limits.MaxConcurrentConnections`、アウトバウンドは `maxconnection`→`SocketsHttpHandler.MaxConnectionsPerServer`＋`IHttpClientFactory`（`ServicePointManager.DefaultConnectionLimit` は Core 無効）。OpenTouryo 対応（net48 は外部 IdP/WS 多用時に `maxconnection`＝auth/oauth2/transmission、タイムアウトは timeout-values、実測は log-analysis）。→ **`app-design/references/concurrency-tuning.md`** 新設＋app-design にチェックリスト行「性能・同時実行（環境）」＋references ポインタ。tok：app-design 2374→2490・concurrency-tuning.md 1353（予算外） |
| **公式「外部パラメタ」照合＝大半既収録・運用注記のみ追記**（作者提示 URL・**2026-07-28**） | ページは appSettings/connectionStrings・`appsettings.json`（Core・`IConfiguration`）・resx・独自ファイル（Singleton+Hashtable）の外部パラメタ概観。**大半は `opentouryo-config` に既収録**（`GetConfigParameter` 全API・app.config/web.config⇄appsettings.json・`InitConfiguration`・`FxContainerization`）。独自 key-value＝**共有情報**（`GetSharedProperty`＝shared-property）、i18n＝message、で既にカバー。**真ギャップ＝運用注記のみ**：設定は起動時に読む/キャッシュ＝**オンライン変更は反映されない→再起動（web.config は IIS が app pool 再起動）／無停止はローリング・アップグレード**（`appsettings.json reloadOnChange` はあるが `GetConfigParameter` は `InitConfiguration` 時の構成を使う＝Fx 系は起動時前提）。→ config 適用範囲に「外部パラメタの置き場（config/共有情報/message の対応表）＋運用注記」を追記。tok：config 3098→3385（上限内） |
| **公式「文字コード、エンコーディング、キャラクタセット」を `character-encoding.md` として整備**（作者提示 URL・**2026-07-28**。`Public/Str/StringChecker.cs`・`JIS2k4Checker.cs` で裏取り＝genuine gap） | 既存スキル未収録の OpenTouryo 文字チェック部品を整備。裏取り：**`StringChecker`**〔`IsInCodePage(input, codePage)`＝可逆エンコード・チェック／`IsShift_Jis`／半角/全角/`IsHiragana`/`IsKatakana`/`IsKanji` 等の文字種判定〕・**`JIS2k4Checker`**〔`CheckSurrogatesPairChar`／`DeleteSurrogatesPairChar`／`GetStringInfo`＝サロゲートペア=絵文字/第3・4水準の検出・除去・文字数〕。※ **ページの `JISX0208_1983Checker` は別クラスでなく `StringChecker.IsInCodePage`/`IsShift_Jis` に相当**（wiki 表記ゆれ・合わせない）。基本方針＝内部 UTF-16/外部 UTF-8＝**UTF-8/Unicode 統一**、JIS 水準〔X0208/X0212/X0213・JIS2004〕を DB/画面/帳票/連携で揃える、HTML SJIS→数値文字参照、DB＝`nvarchar`/MySQL `utf8mb4`（絵文字4バイト）。連携＝`FxSqlEncoding`（config）・UTF-8 BOM（comment-convention）・resx（message）。→ **`app-design/references/character-encoding.md`** 新設＋app-design「国際化・文字コード」行を拡充＋references ポインタ。tok：app-design 2490→2568・character-encoding.md 1188（予算外） |
| **公式「国際化対応項目」を `internationalization.md` として整備（OpenTouryo i18n 部品を裏取り）**（作者提示 URL・**2026-07-28**。`FormatConverter.cs`/`GMTMaster.cs`/message で裏取り） | 文字コードとは別トピックの i18n を整備。対応項目（文言/書式/カレンダー/タイムゾーン/和暦・元号/双方向 RTL）＋.NET API（`CultureInfo` の `CurrentCulture`=書式・`CurrentUICulture`=UI を分ける／`.resx`/`JapaneseCalendar`/UTC 保持）。**★ 推奨 C/S 設計**（UI 表示＝クライアント可変／ログ＝サーバ固定／書式＝クライアント or 統一）。**OpenTouryo の i18n 部品を裏取り**：メッセージ カルチャ〔`FxExceptionMessageCulture`/`MSGDefinition_<culture>.xml`＝message 既収録〕・**`GMTMaster`**〔`Business/Util`・**public**・ローカル⇔UTC 変換。※`MyTimeZone` は internal〕・**`FormatConverter`**〔`Public/Str`・public・`Round_Banker`/`Round_4sya5nyu`/`AddFigure3/4/X` 桁区切り〕・`StringChecker`/`JIS2k4Checker`〔character-encoding〕。和暦は net48 `EnforceJapaneseEraYearRanges`／Core は別機構。→ **`app-design/references/internationalization.md`** 新設＋app-design「国際化・文字コード」行に C/S 設計を追記＋references ポインタ。tok：app-design 2568→2626・internationalization.md 1356（予算外） |
| **公式「画面の構成」を `screen-composition.md` として整備（3プラットフォーム対比＋MVC Razor レイアウトの gap 補充）**（作者提示 URL・**2026-07-28**） | WebForms マスタ／ユーザコントロールは既収録だが、**MVC の Razor レイアウト（`_Layout.cshtml`/`RenderSection`）と3プラットフォーム対比が未収録**（gap）。共通の考え方＝**親〔デザインベース〕＋個別〔子〕＋ユーザコントロール**を実現手段別に対比表化：WebForms＝マスタページ〔`.master`/`BaseMasterController`/`ContentPlaceHolder`/**Fx 隠しフィールド**〕＋`.ascx`／WinForms＝ベース Form＋UserControl／MVC＝`Views/Shared/_Layout.cshtml`＋`RenderSection`＋**HTML ヘルパー**。**★ 先の「Web Forms 専用」テーマと整合**：MVC の `_Layout` に Fx 隠しフィールドは無い＝標準 Razor・OpenTouryo 独自マスタ機構なし（不正操作防止/画面遷移は WebForms 専用）、ボタンも WebForms=UOC 自動結線／MVC=Action。→ **`app-design/references/screen-composition.md`** 新設＋app-design「画面設計」行に「画面構成」＋layer-p-mvc 参照追加。tok：app-design 2626→2697・screen-composition.md 991（予算外） |
| **`opentouryo-layer-p-winforms-event` に「Control の所在別のハンドラ配置」を追記**（作者の問い「十分か」・**2026-07-29**。`BaseControllerWin.cs`〔RichClient〕・`RcMyCmnFunction.cs`・`WSClientWin2_sample` で裏取り） | skill は接頭辞結線・シグネチャは厚いが、**Control が Form 上か UserControl 上かでハンドラを書く場所が変わる**点（WebForms event の content/master/UC 命名表の WinForms 版）が欠けていた＝**不十分**と判断し追補。裏取り（`GetMethodName` L650-666／`CMN_Event_Handler` L684-701／`RcMyCmnFunction` の再帰 L113/195／`LstUserControl` 自動収集 L903-912）：**①Form 上（ベースForm 継承分含む）＝`UOC_(名)_(イベント)`・接頭辞なし**（再帰検索で見つかる＝**WebForms のマスタ名接頭辞は WinForms では不要**）／**②UserControl 上＝UC クラス自身に `UOC_(名)_(イベント)`〔接頭辞なし〕or Form 側に `UOC_(UCのName)_(名)_(イベント)`**、**Form 側が優先**（Form→各 UserControl の順に探索。サンプル Form3 の「UserControl より Form が優先」コメントと一致）。UserControl は自動収集＝手動登録不要・動的追加/削除可。→ 新節「ハンドラをどこに書くか（Control の所在別）」＋description に所在別配置のトリガ語（ベースForm/UserControl）。サンプルは削除されうるので規則を正とする注記。tok：winforms-event 2397→3045（上限内） |
| **公式「多言語化辞書テーブル」「UI コントロールの制御方式」を `table-driven-control.md` として整備**（作者提示 2URL・**2026-07-29**。`UOC_CMNFormInit` 実在で裏取り） | 2トピックの共通＝**親クラス2 `UOC_CMNFormInit`〔全画面共通の初期化・WebForms `BaseController` L3726/WinForms `BaseControllerWin` L1006 の abstract〕で Control を再帰走査し、画面名＋コントロール名キーでテーブルから一括適用する纏め者パターン**（作者の「control を再帰処理で自動処理」と一致）。応用①**多言語化 辞書テーブル**〔画面名/コントロール名/日本語/英語/…・SQL/XML〔`DataTable.WriteXml/ReadXml`〕/DataTable〔`Select`〕。**`.resx` 既定の代替で定義の可視性が良い**〕②**権限・状態制御**〔画面名/コントロール名/ロール/状態/表示・活性→`opentouryo-auth`〕。**★ WebForms/WinForms のみ＝MVC は Control ツリー再帰不可**（先の「Web Forms 専用」テーマと整合）。カスタムコントロールは `Render` 系。実装は纏め者＝base2-customize。→ **`app-design/references/table-driven-control.md`** 新設＋`internationalization.md` に「resx か辞書テーブルか」1行＋app-design「国際化・文字コード」行・references ポインタ。tok：app-design 2697→2763・table-driven-control.md 873・internationalization.md 1356→1461（予算外） |
| **公式「バッチ処理方式」＋実サンプル `Bat_sample` を `batch-processing.md` として整備（未整備領域を解消）**（作者提示 URL＋「Bat_sample とほぼ同じはず」・**2026-07-29**。`SimpleBatch`/`RerunnableBatch` の `Program.cs`/`LayerB` で裏取り＝ページと一致確認） | app-design の未整備「バッチ処理方式」を reference 化。裏取り：**P層＝コンソール `Program.Main`〔専用基底なし〕**＝`StringVariableOperator.GetCommandArgs('/')` でコマンドライン解析→業務 pv 組立→**`new LayerB().DoBusinessLogic(pv, iso)` 直呼び**〔インプロセス＝Web と同型〕。**RerunnableBatch のパターン**＝**主キーだけ先に取得**〔`SelectPkList`・ORDER BY PK・全件メモリ回避〕→ **`INTERMEDIATE_COMMIT_COUNT`〔例100〕ごとに1トランザクション**〔`ExecuteBatchProcess(subPkList)`＝read→処理→write→commit〕→ `initialIndex` で**リラン再開**。ページの**落とし穴6点**〔全件メモリ/ロングTx/リランなし/多重化不可/フェッチ/DB間通信〕と対策が Bat_sample の実装と一致（作者の指摘どおり）。D層技法＝配列バインド〔`DamManagedOdp.ArrayBindCount`〕/バッチSQL〔`SQLUtility`〕/クエリキャッシュ〔batch-update・dao-generated〕。**※ 公式の `DamOraOdp` は表記で実体 `DamManagedOdp`**（既出の是正と一貫）。オンライン起動＝同期 `WaitForExit`/非同期 `Process.Start`・排他 `Semaphore`/`Mutex`。→ **`app-design/references/batch-processing.md`** 新設、app-design「まず処理方式を決める」にバッチ参照追加、**未整備リストから「バッチ処理方式」を除去**、references ポインタ追加。tok：app-design 2763→2794・batch-processing.md 1232（予算外） |
| **「非同期処理サービス」の対応付けを是正（作者が実施）＝別リポジトリ**（作者指摘・**2026-07-29**） | batch-processing.md で「多重度・リトライ・結果通知が要るなら非同期処理サービス」を当初 `opentouryo-richclient-async` に対応付けたのは誤り。**正しくは別プロダクト [AsynchronousProcessingService](https://github.com/OpenTouryoProject/AsynchronousProcessingService)**（非同期処理**サービス**＝多重度/リトライ/結果通知制御）。`opentouryo-richclient-async` は**リッチクライアント（WinForms/WPF）の非同期 B層呼び出し**（`MyBaseAsyncFunc`）で別物。作者が line 53 を正 URL に修正済み。横断確認：「非同期処理サービス」の言及は batch-processing.md の1箇所のみ・richclient-async は description/本文ともサービスを騙っていない＝他に誤りなし。tok：batch-processing.md 1232→1241 |
| **「非同期」3種の正式名と区別を明文化**（作者提示 3URL・**2026-07-29**。`BaseAsyncFunc`/`AsyncEventFx` 実在で裏取り） | 作者提示の正式名：**①非同期呼出フレームワーク**〔リッチクライアントから B層を非同期呼出・`BaseAsyncFunc`/`MyBaseAsyncFunc`・APM/EAP・`Control.Invoke`/`Dispatcher.Invoke`＝**`opentouryo-richclient-async` が正式にこれ**〕／**②非同期イベント・フレームワーク**〔**組込系**・デバイスドライバ通信/IPC/名前付きパイプ/メモリマップト共有メモリ・`AsyncEventFx`〔`Framework/RichClient/Asynchronous/` に実在〕・サンプル `AsyncEvent_sample`＝未収録・組込系未整備〕／**③非同期処理サービス**〔多重度/リトライ/結果通知・RDB キューテーブル・別リポジトリ v02-30 分割〕。→ richclient-async に H1 で正式名・「3種を混同しない」ブロック・実クラス〔`BaseAsyncFunc`+`MyBaseAsyncFunc`〕＋description にトリガ語、app-design「呼出/非同期」行に3種の区別を追記。**memory**〔reference〕を「非同期3種の区別」に拡張（②③は未収録・②は組込系未整備）。tok：richclient-async 1731→2113・app-design 2794→2873（上限内） |
| **公式「共通情報の持ち回り方式」照合＝2経路の統合ビューを state-management に追記**（作者提示 URL・**2026-07-29**） | ページの中身（ユーザ情報クラス＝ユーザ名/端末/権限・ログオン時設定・ASP.NET Session/リッチはグローバル／共通引数クラス＝画面名/コントロール名/メソッド名/処理区分/ユーザ情報を B・D層へ）は**既存スキルに分散収録済み**（`MyUserInfo`＝auth・共通引数＝p-call-business・Session＝state-management・定数＝shared-property）。真の価値＝**「持ち回りは2経路」の統合ビュー**を明文化：**①ユーザ情報クラス `MyUserInfo`→Session/global（ログオン時・`UserInfoHandle`＝auth）②共通引数クラス `MyParameterValue`/`MyReturnValue` 派生→P→B→D 引数（p-call-business・共通項目は親クラス2）**、定数＝共有情報。使い分け（ユーザ状態/レイヤ受け渡し/定数）を明記。→ state-management.md に節「共通情報の持ち回り（2経路）」＋app-design「共有情報・設定・状態管理」行に2経路。tok：app-design 2873→2920・state-management.md 1461→1531（予算外） |
| **`machineKey` の Core 対応を追記（net48 前提だった）**（作者の問い・**2026-07-29**） | state-management.md の負荷分散注記が `<machineKey>`（net48）前提だった。**Core（net10.0）に `machineKey` は無い→ASP.NET Core Data Protection**（`AddDataProtection`＝Cookie 認証・アンチフォージェリ・TempData 等を保護）。既定キーリングはマシン ローカル＝Web ファームでは**共有ストレージに永続化**（`PersistKeysToFileSystem`〔共有UNC〕/`PersistKeysToStackExchangeRedis`/Azure Blob）＋複数アプリ共有は `SetApplicationName`。放置でスケールアウト/再起動時にトークン失効。→ machineKey 行を net48/Core 併記に是正。横断確認：machineKey の言及は state-management.md のみ（他スキルに net48 前提の残存なし）。tok：state-management.md 1531→1720（予算外） |
| **公式「Session 管理機能」照合＝大半既収録・補足3点を追記**（作者提示 URL・**2026-07-29**。`MyCmnFunction.CalculateSessionSize*` 実在で裏取り） | 大半既収録（Session 領域＝親画面/窓別＝webforms-dialog・タイムアウト検出/`IsNoSession`/`FxSessionAbandon`＝auth）。**補足の真ギャップ**：①**Session 領域の自動削除あり/なし**〔あり＝親画面/ブラウザ・ウィンドウ別 LRU／**なし＝ユーザ情報用・サブシステムID別**〔セッション中保持〕〕②**タイムアウト防止（Ping）**〔クライアント定期リクエストで維持。※C# の専用クラスは無く概念〕③**Session サイズ計測 `MyCmnFunction.CalculateSessionSizeMB()`/`KB()`**〔`Business/Util`・public static。Session 肥大の監視＝batch-update のメモリ懸念と接続〕。→ state-management.md に「OpenTouryo の Session 管理機能（補足）」節。tok：state-management.md 1720→2015（予算外） |
| **公式「Webアプリケーションの画面遷移」照合＝Redirect/Transfer 設計比較を screen-transition に追記**（作者提示 URL・**2026-07-29**） | 大半既収録（`FxRedirect`/`FxTransfer`・`ScreenTransitionMethod` 1/2＝screen-transition・POST/GET 再送＝illegal-operation・持ち回り＝state-management）。**真ギャップ＝Redirect vs Transfer の設計比較**を表化：**Redirect**〔`Response.Redirect`・URL 変わる・1対1・リロード=最後のリクエスト再実行=GET 再送・持ち回り=Session〕／**Transfer**〔`Server.Transfer`・URL 変わらない・リロード=前のリクエスト再実行=POST 再送・持ち回り=`HttpContext.Items`/Hidden・性能有利〕。**★ Transfer＋Hidden/HttpContext でステートレス設計（`IsNoSession`）**。**MVC は Transfer 無し**〔`RedirectToAction`/`Redirect(Url.Action)`・`Html.BeginForm`=自コントローラ Post/View 再描画・`Ajax.BeginForm`=PartialView〕。リロード挙動→illegal-operation、持ち回り→state-management に相互参照。※TLS〔`ThreadStaticAttribute`/`AllocateDataSlot`〕は基盤のみ・利用困難で取り込まず。tok：screen-transition 2010→2388（上限内） |
| **公式「例外処理方式」「エラー・メッセージ共通編集」再確認＝ほぼ既収録・ログレベル既定のみ追記**（作者「既出・再確認」・**2026-07-29**。`MyFcBaseLogic.UOC_ABEND` で裏取り） | 2ページとも既存スキルで手厚く収録済みを確認：**例外処理方式**＝3分類〔業務`BusinessApplicationException`/システム`BusinessSystemException`/その他〕・リスロー〔業務は飛ばず ErrorFlag〕・UOC_ABEND 振替表・閉塞・自動ロールバックは `opentouryo-exception`／**エラー・メッセージ共通編集**の3方式〔①固定メッセージ②雛形〔MSGDefinition〕③可変〕は `opentouryo-message`〔`%1/%2`・完成文・カルチャ〕。**唯一薄かった「例外種別ごとのログレベル・表示先」を追記**（実ソース裏取り：既定 `MyFcBaseLogic.UOC_ABEND`＝業務→`WarnLog`〔L417〕/システム→`ErrorLog`〔L472〕/その他→振替次第 Warn or Error・`ACCESS` ロガー。表示先＝業務はメッセージエリア/その他は共通エラー画面。**親クラス2 で決まる**）＝ページのログレベル表と一致。→ exception に1段落。tok：exception 4250→4427（上限内） |
| **公式「閉塞・閉塞チェック方式」を table-driven-control.md③として整備（base2 は上限直下で非拡張）**（作者「base2-customize 案件」・**2026-07-29**。全 P層親クラス2 の `// 閉塞チェック` stub 実在で裏取り） | 閉塞は 権限・状態と同じ**親クラス2 のテーブル駆動制御**。裏取り：**全 P層方式の親クラス2 に `// 権限チェック`・`// 閉塞チェック` の空 stub が実在**（`MyBaseController` L232/434〔WebForms〕・`MyBaseMVController(Core)`〔MVC〕・`MyBaseAsyncApiController(Core)`〔Web API〕・`MyBaseControllerWin` L389/513〔WinForms〕）。纏め者がここに**閉塞テーブル**〔機能/画面/イベント単位×閉塞状態〕参照を実装。種類＝障害閉塞〔エラー起点〕/運用閉塞〔メンテ/バッチ〕。閉塞なら**業務 or システム例外に閉塞用 messageID をセットしてスロー**→`UOC_ABEND` 振替（exception）。**★ 閉塞は Control 再帰でなく「テーブル引き＋例外」なので MVC/Web API でも可**（①②の Control 再帰＝WebForms/WinForms のみ、と違う）。→ **`table-driven-control.md` に③閉塞制御**〔タイトルを「親クラス2 のテーブル駆動制御（多言語化・UI 制御・閉塞）」に一般化〕、**exception の閉塞箇所に実装ポインタ**。**base2 は tok~4989 で上限直下＝本文非拡張**（Web API 行で閉塞 stub 既述・詳細は table-driven-control へ委譲）。tok：table-driven-control.md 873→1320・exception 4427→4569（base2 は 4989 据置） |
...
### 4.6 作者レビューの傾向（自己チェック観点）

作者の**直接の指摘・提案**（エージェントの field-test レポートを除く）を通じて繰り返し矯正された観点。**着手前・提出前のセルフチェックに使う。**
一貫する軸は「**正確さ／正しい配置／体系化／簡潔さ／一貫性・網羅**」、通底するメタ指示は **「一件直したら類似を洗え」**。傾向として、私は
*断定しすぎ・置き場所を誤る・ノイズを残す・局所対応で済ませる* 癖があり、そこを一貫して矯正されている。

- **① 断定しすぎない（裏取り）** — エビデンスを超えて書かない。サンプルの慣習をフレームワーク規則として書かない。例：「2CS は2本目の接続も可」の根拠不足（二度是正）／MVC の `ActionName` はサンプル都合／`this.GetDam()` でも手動制御可。
- **② 正しい置き場所・分類** — メソッドとプロパティを同列に並べない（`ClearParameters()` と `CommandTimeout`）。概念のバケツを間違えない（楽観排他は「混在の注意」ではない→独立節）。汎用の内容を特定スキルに閉じ込めない（IDENTITY／楽観排他は batch-update でなく `opentouryo-layer-d`）。
- **③ 局所でなく体系的に** — 一件の欠陥を見つけたら**同型を全スキル横断で洗う**（「類似の問題はあるか」「類似見直しも」）。症状でなく「クラス」を直す。**最も繰り返されるメタ指示。**
- **④ 冗長・ノイズを削る** — 存在しないものを名指して「無い」と書かない（`buttonHistoryRecorder`／`DamOraOdp 旧称`）。「実測」等の強調は自明な所では外す（必要最小限）。本文とスニペットの重複はスニペットへ退避。
- **⑤ 一貫性・並列・導線** — 横並びのもの（GridView／ListView／Repeater 等）は**同構造・同粒度**にし、SKILL から相互リンクする。同名で混同させない（master と aspx は別名）。
- **⑥ 網羅・完全** — 対になるものを欠かさない（`.aspx` には code-behind も）。**選択肢は間引かない**（全系列・全候補を提示）。
- **⑦ 先回りの設計** — 運用で起きうる衝突を仕組みで塞ぐ（既設 `OT_RESOURCE_ROOT` は別名／横断ルールは集約より **defense-in-depth の重複**）。

導出：本セッション（〜**2026-07-26**）の作者指摘を集約。field-test レポートの反映手順は repo 直下 `AGENTS.md`「作者の検証実行（field-test）レポートの反映手順」を参照（住み分け）。
...
**このリポジトリ側の作業は残っていない。** 全39スキル（`opentouryo-batch-update`・`opentouryo-webforms-crud-screens`・`opentouryo-log-analysis`・`opentouryo-comment-convention`・`opentouryo-app-design` を含む）、
