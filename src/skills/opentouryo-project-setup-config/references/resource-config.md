...
**検証（動作確認）だけ**なら User 変数を変えず、**起動プロセスに限定して上書き**すれば足りる（`$env:OT_RESOURCE_ROOT="<repo>\resource"`
を起動コマンドで指定＝`run-verify.md`）。他リポジトリに影響せず `SETUP-CHANGES.md` も不要。
**恒久的に本リポジトリで使う**ときだけ、既設と奪い合わないよう**別名にする**（例 `OT_RESOURCE_ROOT2`・またはプロジェクト固有名）：
このプロジェクトの config 各キーと log4net の `%env{...}%` を**その別名**に張り替え、どちらの名前を使ったかを `SETUP-CHANGES.md` に記録する。
